 ...
 */
class TouchCursorPlugin extends FlxTypedSpriteGroup<TouchCursor>
{
  /**
   ...
   */
  static var instance:TouchCursorPlugin = null;
  ...
  public static function initialize():Void
  {
    ...
    instance = new TouchCursorPlugin();
  ...
  override public function update(elapsed:Float):Void
  {
    ...
    for (touch in FlxG.touches.list)
    {
      ...
      var pointer:TouchCursor = findPointerByTouchId(touch.touchPointID);

      if (pointer == null)
      {
        pointer = recycle(TouchCursor);
   ...
   */
  function findPointerByTouchId(touchId:Int):TouchCursor
  {
    for (pointer in members)
    {
...
class TouchCursor extends FlxSprite
{
  ...
  public function updateFromTouch(touch:FlxTouch, camera:FlxCamera):Void
  {
    ...
    final deltaX:Float = viewPoint.x - lastPosition.x;
    final deltaY:Float = viewPoint.y - lastPosition.y;
    final isMoving:Bool = deltaX * deltaX + deltaY * deltaY > 9;
    ...
    {
      moving = isMoving;
      loadGraphic(moving ? 'assets/images/cursor/kevin.png' : 'assets/images/cursor/michael.png');
    }
