...
class PauseSubState extends MusicBeatSubState
{
  ...
  function regenerateMenu(?targetMode:PauseMode):Void
  {
    ...
    PlayState.instance?.applyLuaPauseMenuHooks(this);
    #end
  ...
  #if FEATURE_LUA_SCRIPTS
  public function getLuaPauseMenuMode():String
  {
  ...
  function applyLuaPauseMenuItem(item:Dynamic, optionsConfig:Dynamic, customCallback:String->Void):Void
  {
    ...
    if (hidden)
    {
      if (entry != null) currentMenuEntries.remove(entry);
      return;
    }
    ...
    if (entry == null)
    {
      entry = {text: label, callback: callback};
      currentMenuEntries.push(entry);
    }
    else
    {
      entry.text = label;
